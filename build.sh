...
# Depends on apt packages: libguestfs-tools qemu-utils systemd-container 
# Also assumes `iptables` (legacy or iptables-nft) is being used; if not, dependent lines are not necessary.
...
mount_image() {
	...
	mkdir -p "$mnt"
	mount --label "cloudimg-rootfs" "$mnt"
}
